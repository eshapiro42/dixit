...
def createGame():
    ...
    return 'Created game {}'.format(game_id)
...
def joinGame(player_name=None, game_id=None):
    ...
    return 'Player {} joined game {}'.format(player_name, game_id)


@app.route('/api/showHand', methods=['GET'])
def showHand(game_id=None, player_num=None):
    if game_id is None:
        game_id = request.form['game_id']
